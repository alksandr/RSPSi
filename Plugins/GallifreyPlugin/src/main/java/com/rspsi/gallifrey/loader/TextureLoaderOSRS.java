...
public class TextureLoaderOSRS extends TextureLoader {
	...
	public int[] getPixels(int textureId) {
		if(textureId < 0) // -1 = untextured face; queried constantly by the renderer
			return null;
	...
	public void init(Archive archive, Index spriteIndex) {
		...
		for(File file : archive.files()) {
			if(file != null && file.getData() != null) {
				...
				// RuneLite TextureLoader format: field1777 (avg color) + field1778 (bool) + count + fileIds
				buffer.readUnsignedShort();          // field1777 average color (unused)
				buffer.readUnsignedByte();           // field1778 boolean flag (unused)
				int count = buffer.readUnsignedByte();
				if (count < 1)
					continue;
				int[] fileIds = new int[count];
				for (int i = 0; i < count; i++)
					fileIds[i] = buffer.readUnsignedShort();
				int spriteId = fileIds[0];           // first sprite drives the texture
				Archive spriteArchive = spriteIndex.archive(spriteId);
				if (spriteArchive == null || spriteArchive.file(0) == null || spriteArchive.file(0).getData() == null) {
					log.warn("Missing sprite {} for texture {}", spriteId, file.getId());
					continue;
				}
				Sprite sprite = Sprite.decode(ByteBuffer.wrap(spriteArchive.file(0).getData()));
