...
public class ObjectDefinitionLoaderOSRS extends ObjectDefinitionLoader {
	...
	/**
	 * Initialise from raw file bytes read by {@link GallifreyRawCache} (bypasses displee,
	 * which cannot split this cache's objects archive). Keyed by object id.
	 */
	public void init(Map<Integer, byte[]> files) {
		int highestId = 0;
		...
		for (Map.Entry<Integer, byte[]> entry : files.entrySet()) {
			...
			try {
				ObjectDefinition def = decode(entry.getKey(), new Buffer(data));
				cache.put(entry.getKey(), def);
			} catch (Exception ex) {
				ex.printStackTrace();
			}
		}
	...
	public ObjectDefinition decode(int id, Buffer buffer) {
		...
		do {
			...
			if (opcode == 2) {
				...
				// GallifreyClient (RuneLite) format: ushort model id + ubyte type per entry
				...
				// ushort model id, no per-model type
				int count = buffer.readUByte();
				if (count > 0) {
					if (definition.getModelIds() == null) {
				...
				// GallifreyClient (RuneLite) uses 5 actions: opcodes 30-34
