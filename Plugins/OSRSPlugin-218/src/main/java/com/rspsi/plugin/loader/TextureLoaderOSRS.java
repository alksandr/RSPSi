		...
		for(File file : archive.files()) {
			if(file != null && file.getData() != null) {
				try {
					...
					buffer.readUnsignedShort(); // field1777 (average color, skip)
					buffer.readByte();          // field1778 (boolean flag, skip)
					int spriteCount = buffer.readUByte();
					if (spriteCount <= 0) continue;
					int spriteId = buffer.readUnsignedShort(); // first file ID = actual sprite ID
					...
					if (spriteArchive == null || spriteArchive.file(0) == null || spriteArchive.file(0).getData() == null) {
						log.warn("Texture {} references missing sprite {}", file.getId(), spriteId);
						continue;
					}
					...
					log.warn("Failed to load texture {}: {}", file.getId(), ex.getMessage());
				}
			}
		}
	}
