	...
	public ObjectDefinition decode(int id, Buffer buffer) {
		...
		do {
			...
			if (opcode == 1) {
				...
				definition.setTranslateX(buffer.readUShort());
			} else if (opcode == 71) {
				definition.setTranslateY(buffer.readUShort());
			} else if (opcode == 72) {
				definition.setTranslateZ(buffer.readUShort());
			} else if (opcode == 73) {
				...
				buffer.readUShort(); // ambientSoundId
				buffer.readUByte(); // ambientSoundDistance
			} else if (opcode == 79) {
				buffer.readUShort(); // ambientSoundChangeTicksMin
				buffer.readUShort(); // ambientSoundChangeTicksMax
				buffer.readUByte(); // ambientSoundDistance
				int count = buffer.readUByte();
				for (int i = 0; i < count; i++) {
					buffer.readUShort(); // ambientSoundIds
				}
			} else if (opcode == 81) {
